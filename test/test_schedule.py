...
class ScheduleTestCase(TestCase):
    def test_events(self):
        ...
        self.assertTrue(
            qs.match(lon.localize(datetime(2012, 12, 25, 15, 0))),
            'Queen on time in London')
        self.assertTrue(
            qs.match(van.localize(datetime(2011, 12, 25, 7, 0))),
            'Queen on time in Vancouver last year')
        self.assertTrue(
            qs.match(syd.localize(datetime(2013, 12, 26, 2, 0))),
            'Queen on time in Sydney next year')
        self.assertTrue(
            qs.match(datetime(2020, 12, 25, 15, 0, tzinfo=UTC)),
            'Queen on time in UTC')

        self.assertTrue(
            ar.match(van.localize(datetime(2012, 11, 11, 11, 0))),
            'Armistice correct time in Vancouver')
        self.assertTrue(
            ar.match(lon.localize(datetime(2012, 11, 11, 19, 0))),
            'Vancouver Armistice correct time in London')
        self.assertTrue(
            ar.match(syd.localize(datetime(2012, 11, 12, 6, 0))),
            'Vancouver Armistice correct time in Sydney')

        # Check scheduling tests all date parts.

        self.assertFalse(
            qs.match(lon.localize(datetime(2012, 12, 25, 14, 0))),
            'Queen not one hour early')
        self.assertFalse(
            qs.match(lon.localize(datetime(2012, 12, 25, 15, 5))),
            'Queen not five minutes late')
        self.assertFalse(
            qs.match(lon.localize(datetime(2012, 12, 26, 15, 0))),
            'Queen not on boxing day')
        self.assertFalse(
            qs.match(lon.localize(datetime(2012, 11, 25, 15, 0))),
            'Queen not on in November')

        # And an event which doesn't happen at minute zero.

        self.assertTrue(
            gf.match(lon.localize(datetime(2012, 11, 5, 19, 30, 0))),
            'Fireworks on time')
        self.assertTrue(
            gf.match(lon.localize(datetime(2012, 11, 5, 19, 30, 1))),
            'Fireworks might be one second late')
        self.assertFalse(
            gf.match(lon.localize(datetime(2012, 11, 5, 19, 35))),
    ...
    def test_range(self):
        ...
        self.assertTrue(
            fm.match(datetime(2012, 7, 4, 10, 0, tzinfo=UTC)),
            'Minute 0 % 5')
        self.assertFalse(
            fm.match(datetime(2013, 8, 5, 14, 1, tzinfo=UTC)),
            'Minute 1 not % 5')
        self.assertFalse(
            fm.match(datetime(2014, 9, 7, 18, 4, tzinfo=UTC)),
            'Minute 4 not % 5')
        self.assertTrue(
            fm.match(datetime(2015, 1, 9, 20, 10, tzinfo=UTC)),
            'Minute 10 % 5')
        self.assertFalse(
            fm.match(datetime(2010, 2, 1, 11, 42, tzinfo=UTC)),
            'Minute 42 not % 5')
        self.assertTrue(
            fm.match(datetime(2011, 3, 2, 13, 45, tzinfo=UTC)),
            'Minute 45 % 5')
        self.assertTrue(
            fm.match(datetime(2012, 4, 3, 16, 55, tzinfo=UTC)),
            'Minute 55 % 5')
        self.assertFalse(
            fm.match(datetime(2012, 5, 4, 19, 59, tzinfo=UTC)),
            'Minute 59 not % 5')

        wd = CrabSchedule('0 0 * * 1-5', 'UTC')
        self.assertTrue(
            wd.match(datetime(2012, 5, 7, 0, 0, tzinfo=UTC)),
            'May 7th 2012 a weekday')
        self.assertTrue(
            wd.match(datetime(2012, 5, 8, 0, 0, tzinfo=UTC)),
            'May 8th 2012 a weekday')
        self.assertTrue(
            wd.match(datetime(2012, 5, 9, 0, 0, tzinfo=UTC)),
            'May 9th 2012 a weekday')
        self.assertTrue(
            wd.match(datetime(2012, 5, 10, 0, 0, tzinfo=UTC)),
            'May 10th 2012 a weekday')
        self.assertTrue(
            wd.match(datetime(2012, 5, 11, 0, 0, tzinfo=UTC)),
            'May 11th 2012 a weekday')
        self.assertFalse(
            wd.match(datetime(2012, 5, 12, 0, 0, tzinfo=UTC)),
            'May 12th 2012 not a weekday')
        self.assertFalse(
            wd.match(datetime(2012, 5, 13, 0, 0, tzinfo=UTC)),
    ...
    def test_list(self):
        ...
        self.assertTrue(
            br.match(hon.localize(datetime(2012, 10, 15, 10, 0))),
            'Coffee break on time')
        self.assertTrue(
            br.match(hon.localize(datetime(2012, 10, 15, 15, 0))),
            'Tea break on time')
        self.assertFalse(
            br.match(hon.localize(datetime(2012, 10, 15, 11, 0))),
            'Coffee break not 1 hour late')
        self.assertFalse(
            br.match(hon.localize(datetime(2012, 10, 15, 16, 0))),
    ...
    def test_aliases(self):
        ...
        self.assertTrue(
            ho.match(datetime(1998, 10, 30, 1, 0, tzinfo=UTC)),
            'Hourly alias match')
        self.assertFalse(
            ho.match(datetime(1998, 10, 30, 1, 1, tzinfo=UTC)),
            'Hourly alias non-match')
        self.assertTrue(
            da.match(datetime(1998, 10, 30, 0, 0, tzinfo=UTC)),
            'Daily alias match')
        self.assertFalse(
            da.match(datetime(1998, 10, 30, 1, 0, tzinfo=UTC)),
            'Daily alias non-match')
        self.assertTrue(
            wk.match(datetime(1998, 10, 25, 0, 0, tzinfo=UTC)),
            'Weekly alias match')
        self.assertFalse(
            wk.match(datetime(1998, 10, 30, 0, 0, tzinfo=UTC)),
